...
class XMLWalkerSection:
    ...
    def __init__(self, transmogrifier, name, options, previous):
        ...
        self.parentkey = Expression(
            options.get("parent-key", "string:_parent"), transmogrifier, name, options
        )

        # By default, only insert a type for folders
        self.typekey = Expression(
            options.get("type-key", "string:_type"), transmogrifier, name, options
        )
        self.typevalue = Expression(
            options.get("type-value", "string:Folder"), transmogrifier, name, options
        )

        # By default yield default pages for folders
        #   but do not keep references to child items
        self.defaultpagekey = Expression(
            options.get("default-page-key", "nothing"), transmogrifier, name, options
        )
        ...
        self.childrenkey = Expression(
            options.get("children-key", "nothing"), transmogrifier, name, options
        )

        self.keys = [
            (key, Expression(options[self.prefix + key], transmogrifier, name, options))
        ...
        for item in self.previous:
            ...
            for tree in trees:
                if not (
                    ...
                    tree = html.fragment_fromstring(tree, create_parent=True)
                if self.cache:
                ...
                if element.xpath(self.xpath):
                    ...
                    if previous_depth == 0:
                        ...
                        if isdefaultpagekey:
                            ...
                            if childrenkey:
                                previous.setdefault(childrenkey, []).append(defaultpage)
        ...
        if previous_depth != 0:
            ...
            previous.update(
                (
                    key,
                    expression(
                        previous, element=previous_element, source_item=item, tree=tree
                    ),
                )
                for key, expression in self.keys
            )
